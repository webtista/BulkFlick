...
export async function ensureLogin(parent?: BrowserWindow): Promise<OAuthToken> {
  ...
  if (!oauth_token || !oauth_token_secret) throw new Error("Failed to obtain request token");
  ...
  if (!token.oauth_token || !token.oauth_token_secret) throw new Error("Failed to obtain access token");
...
export async function getAlbumPhotos(photosetId: string, sizePref: SizePref): Promise<DownloadImage[]> {
  ...
  do {
    ...
    for (const p of photos) {
      ...
      if (best) result.push({ id: p.id, title: p.title, bestUrl: best });
    }
